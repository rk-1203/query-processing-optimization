	    ...
	    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException 
	    {
	    	...
	    	for(String x : propList)
	    	{
	    		...
	    		if(propSet.contains(list[0].trim()))
	    		{
	    			...
	    			{
	    				ArrayList<String> temp = map.get(list[0].trim());
	    				temp.add(list[1].trim());
	    				map.put(list[0].trim(),temp);
	    			}
	    			else
	    			{
	    	...
	    	if(flag==1)
	    	{
	    		...
	    		for(HashMap.Entry<String,ArrayList<String>> entry : map.entrySet())
	    		{
	    			...
	    			t = t.startsWith("<") && t.endsWith(">")
	         				...
	         				: t.replaceAll("[[^\\w]+]", "_");
	         		if(t.contains("emailAddress")||t.contains("memberOf")||t.contains("telephone"))
		    			record.put(t,entry.getValue().get(0));
	         		else
	         			record.put(t,entry.getValue());
	    		}
	    ...
	    for(String x : naryPropTable)
        {
        	...
        	for( String t : temp)
        	{
        		t = t.startsWith("<") && t.endsWith(">")
        				? t.substring(1, t.length() - 1).replaceAll("[[^\\w]+]", "_")
        				: t.replaceAll("[[^\\w]+]", "_");
        		//s=s+"    {\"name\":\""+t+"\", \"type\":[\"null\",{ \"type\": \"array\", \"items\": \"string\"}]},\n";	
				if(t.contains("emailAddress")||t.contains("memberOf")||t.contains("telephone"))
	        	{
					s=s+"    {\"name\":\""+t+"\", \"type\":[\"null\",\"string\"]},\n";	
	        	}
	        	else
	        	{
	        		s=s+"    {\"name\":\""+t+"\", \"type\":[\"null\",{ \"type\": \"array\", \"items\": \"string\"}]},\n";	
            ...
            FileOutputFormat.setOutputPath(job_makeParquet, new Path(new String(args[1]+"/"+j)));
